...
func TestSigle(t *testing.T) {
	p := NewPool("192.168.0.106:6379", "", 10, 10, 10)
	c := p.Pop()
...
func TestPipeSmallBuffer(t *testing.T) {
	c, e := Dial("192.168.0.113:6379", "", ConnectTimeout, ReadTimeout, WriteTimeout, false, nil)
	if e != nil {
		println(e.Error())
		return
	}
...
func TestPoolNoAuth(t *testing.T) {
	p := NewPool("192.168.0.113:6379", "", 10, 10, 10)
